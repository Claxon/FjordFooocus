...
class AsyncTask:
    def __init__(self, args):
        ...
        self.outpaint_expansion = args.pop()
...
def worker():
    ...
    def process_task(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path,
                     controlnet_dwpose_path, current_task_id, denoising_strength, final_scheduler_name,
                     goals, initial_latent, steps, switch, positive_cond, negative_cond, task, loras,
                     tiled, use_expansion, width, height, base_progress, preparation_steps,
        ...
        if 'cn' in goals:
            for cn_flag, cn_path in [
                ...
                (flags.cn_dwpose, controlnet_dwpose_path)
            ]:
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras, persist_image=True) -> list:
        ...
        for x in imgs:
            d = [('Prompt', 'prompt', task['log_positive_prompt']),
                 ...
                 ('FjordFooocus V2 Expansion', 'prompt_expansion', task['expansion']),
                 ('Styles', 'styles',
    ...
    def apply_control_nets(async_task, height, ip_adapter_face_path, ip_adapter_path, width, current_progress):
        ...
        for task in async_task.cn_tasks[flags.cn_cpds]:
            ...
            if async_task.debugging_cn_preprocessor:
                yield_result(async_task, cn_img, current_progress, async_task.black_out_nsfw, do_not_show_finished_images=True)
        for task in async_task.cn_tasks[flags.cn_dwpose]:
            cn_img, cn_stop, cn_weight = task
            cn_img = resize_image(HWC3(cn_img), width=width, height=height)
    ...
    def apply_outpaint(async_task, inpaint_image, inpaint_mask):
        if len(async_task.outpaint_selections) > 0:
            expansion = float(async_task.outpaint_expansion) / 100.0
            H, W, C = inpaint_image.shape
            if 'top' in async_task.outpaint_selections:
                inpaint_image = np.pad(inpaint_image, [[int(H * expansion), 0], [0, 0], [0, 0]], mode='edge')
                inpaint_mask = np.pad(inpaint_mask, [[int(H * expansion), 0], [0, 0]], mode='constant',
                                      constant_values=255)
            if 'bottom' in async_task.outpaint_selections:
                inpaint_image = np.pad(inpaint_image, [[0, int(H * expansion)], [0, 0], [0, 0]], mode='edge')
                inpaint_mask = np.pad(inpaint_mask, [[0, int(H * expansion)], [0, 0]], mode='constant',
                                      constant_values=255)

            H, W, C = inpaint_image.shape
            if 'left' in async_task.outpaint_selections:
                inpaint_image = np.pad(inpaint_image, [[0, 0], [int(W * expansion), 0], [0, 0]], mode='edge')
                inpaint_mask = np.pad(inpaint_mask, [[0, 0], [int(W * expansion), 0]], mode='constant',
                                      constant_values=255)
            if 'right' in async_task.outpaint_selections:
                inpaint_image = np.pad(inpaint_image, [[0, 0], [0, int(W * expansion)], [0, 0]], mode='edge')
                inpaint_mask = np.pad(inpaint_mask, [[0, 0], [0, int(W * expansion)]], mode='constant',
                                      constant_values=255)
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        if use_expansion:
            ...
            for i, t in enumerate(tasks):
                ...
                progressbar(async_task, current_progress, f'Preparing FjordFooocus text #{i + 1} ...')
    ...
    def apply_image_input(async_task, base_model_additional_loras, clip_vision_path, controlnet_canny_path,
                          controlnet_cpds_path, controlnet_dwpose_path, goals, inpaint_head_model_path,
                          inpaint_image, inpaint_mask, inpaint_parameterized, ip_adapter_face_path,
        ...
        if async_task.current_tab == 'ip' or \
            ...
            if len(async_task.cn_tasks[flags.cn_dwpose]) > 0:
                controlnet_dwpose_path = modules.config.downloading_controlnet_dwpose()
                modules.config.downloading_dwpose_detector()
        ...
        return base_model_additional_loras, clip_vision_path, controlnet_canny_path, controlnet_cpds_path, controlnet_dwpose_path, inpaint_head_model_path, inpaint_image, inpaint_mask, ip_adapter_face_path, ip_adapter_path, ip_negative_path, skip_prompt_processing, use_synthetic_refiner
    ...
    def process_enhance(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path,
                        controlnet_dwpose_path, current_progress, current_task_id, denoising_strength, inpaint_disable_initial_latent,
        ...
        imgs, img_paths, current_progress = process_task(all_steps, async_task, callback, controlnet_canny_path,
                                                         controlnet_cpds_path, controlnet_dwpose_path,
    ...
    def enhance_upscale(all_steps, async_task, base_progress, callback, controlnet_canny_path, controlnet_cpds_path,
                        controlnet_dwpose_path, current_task_id, denoising_strength, done_steps_inpainting, done_steps_upscaling, enhance_steps,
    ...
    def handler(async_task: AsyncTask):
        ...
        ar_match = re.search(r'(\d+)\u00d7(\d+)', async_task.aspect_ratios_selection)
        ...
        if async_task.input_image_checkbox:
            base_model_additional_loras, clip_vision_path, controlnet_canny_path, controlnet_cpds_path, controlnet_dwpose_path, inpaint_head_model_path, inpaint_image, inpaint_mask, ip_adapter_face_path, ip_adapter_path, ip_negative_path, skip_prompt_processing, use_synthetic_refiner = apply_image_input(
                ...
                controlnet_dwpose_path, goals, inpaint_head_model_path, inpaint_image, inpaint_mask, inpaint_parameterized,
                ip_adapter_face_path, ip_adapter_path, ip_negative_path, skip_prompt_processing, use_synthetic_refiner)

        # Load or unload CNs
        progressbar(async_task, current_progress, 'Loading control models ...')
        pipeline.refresh_controlnets([controlnet_canny_path, controlnet_cpds_path, controlnet_dwpose_path])
            ...
            for enhance_mask_dino_prompt_text, enhance_prompt, enhance_negative_prompt, enhance_mask_model, enhance_mask_cloth_category, enhance_mask_sam_model, enhance_mask_text_threshold, enhance_mask_box_threshold, enhance_mask_sam_max_detections, enhance_inpaint_disable_initial_latent, enhance_inpaint_engine, enhance_inpaint_strength, enhance_inpaint_respective_field, enhance_inpaint_erode_or_dilate, enhance_mask_invert in async_task.enhance_ctrls:
                ...
                try:
                    current_progress, img, enhance_prompt_processed, enhance_negative_prompt_processed = process_enhance(
                        ...
                        controlnet_dwpose_path, current_progress, current_task_id, denoising_strength, enhance_inpaint_disable_initial_latent,
